namespace MQtoUnity {
	...
	public static class QbFile {
		...
		public static byte[] GetQbByte (QbData _data) {
			...
			mainBytes.AddRange(BitConverter.GetBytes((int)0));
		...
		public static QbData LoadQb (byte[] _data) {
			...
			using (MemoryStream _ms = new MemoryStream(_data)) {
				...
				using (BinaryReader _br = new BinaryReader(_ms)) {
					...
					_mainData.MatrixList = new List<QbMatrix>();


					for (int i = 0; i < _mainData.NumMatrixes; i++) {

						QbMatrix qm = new QbMatrix();
					...
					for (int i = 0; i < _mainData.NumMatrixes; i++) {
						QbMatrix qm = _mainData.MatrixList[i];
						if (i == 0) {
		...
		public static QbData LoadQb (string _path) {
			byte[] _bytes = File.ReadAllBytes(_path);
			return LoadQb(_bytes);
		}


		public static void SaveQb (string _path, QbData _data) {
			byte[] _byte = GetQbByte(_data);
